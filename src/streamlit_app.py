...
# Configure page
st.set_page_config(
    page_title="Drug Safety & Efficacy Analysis Dashboard",
    page_icon="💊",
    layout="wide",
...
def show_overview(df, metadata):
    ...
    if 'safety_score' in df.columns and 'efficacy_score' in df.columns:
        ...
        fig.update_xaxis(title="Safety Score (Higher = Safer)")
        fig.update_yaxis(title="Efficacy Score (Higher = More Effective)")
        st.plotly_chart(fig, use_container_width=True)
